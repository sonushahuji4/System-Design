...
abstract class FileBasedConfigurationManager {
    protected readonly properties: { [key: string]: string };
    ...
    load(filePath: string): void {
        try {
            ...
            const parsedProperties = fileContent.split('\n').reduce((acc: { [key: string]: string }, line) => {
            ...
            Object.assign(this.properties, parsedProperties);
        } catch (error) {
            throw new Error("Error while loading configuration file: " + error);
        }
    }

    static getInstance(): FileBasedConfigurationManager {
    ...
    abstract getConfiguration(key: string): string | undefined;

    abstract getConfiguration<T>(key: string, type: { new(value: string): T }): T | undefined;

    abstract setConfiguration(key: string, value: string): void;

    abstract setConfiguration<T>(key: string, value: T): void;

    abstract removeConfiguration(key: string): void;

    abstract clear(): void;

    protected convert<T>(value: string, type: { new(value: string): T }): T {
        console.log(`Converting ${value} to ${type.name}`);
        switch (type.name) {
            case 'Number':
                return new type(parseFloat(value)) as T;
            default:
...
class FileBasedConfigurationManagerImpl extends FileBasedConfigurationManager {
    private static instance: FileBasedConfigurationManagerImpl | null = null;

    private constructor() {
        super();
    }

    static getInstance(): FileBasedConfigurationManagerImpl {
        if (!FileBasedConfigurationManagerImpl.instance) {
    ...
    getConfiguration<T>(key: string, type: { new(value: string): T }): T | undefined {
        const value = this.getConfiguration(key);
        return value ? new type(value) : undefined;
    }
    ...
    setConfiguration<T>(key: string, value: T): void {
        this.setConfiguration(key, value.toString());
    }
