...
export class DatabaseConnectionPool implements DbConnectionPool{
    private static connectionPool : DatabaseConnectionPool | null = null;
    private maxConnectionPool: number;
    private dbConnectionPool: any[];

    private constructor(maxConnectionPool: number){
        this.maxConnectionPool = maxConnectionPool;
        this.dbConnectionPool = [];
        this.initializePool();
    }

    public static getInstance(maxConnectionPool: number): DatabaseConnectionPool {
        if(!DatabaseConnectionPool.connectionPool){
            DatabaseConnectionPool.connectionPool = new DatabaseConnectionPool(maxConnectionPool);
        }
        return DatabaseConnectionPool.connectionPool;
    }
    ...
    public initializePool(): void{
        for(let pool = 0; pool < this.maxConnectionPool; pool++){
            this.dbConnectionPool.push(new DatabaseConnection());
        }
    }

    @Override
    public getConnection(): DatabaseConnectionPool {
        return this.dbConnectionPool.pop();
    }

    @Override
    public releaseConnection(connection: DatabaseConnection): void {
        this.dbConnectionPool.push(connection);
    }

    @Override
    public getAvailableConnectionsCount(): number {
        return this.dbConnectionPool.length;
    }
